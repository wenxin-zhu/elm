			...
			const login = () => {
				...
				axios.post('UserController/getUserByIdByPass', qs.stringify({
					...
					if (!user) {
						...
						//跳转设置，前往aAI，积分，历史订单与我的功能不回退跳转
						let rpath = sessionStorage.getItem('redirectPath');
						if (sessionStorage.getItem('redirectPath') === '/orderList' || sessionStorage.getItem(
								'redirectPath') === '/myScore'||sessionStorage.getItem('redirectPath')=== '/myInfo'||sessionStorage.getItem('redirectPath')=== '/myAI'){
